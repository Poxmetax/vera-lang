...
impl Parser {
            ...
            if self.at(&["::"]) {
                ...
                if named {
                    ...
                    let name = match &expr {
                        ...
                        _ => {
                            return Err(ParseError {
                                message: "named args only allowed for struct construction".into(),
                                span,
                            });
                        }
                    };
                    expr = Expr::StructLit { name, fields, span };
                } else {
                    let mut args = Vec::new();
                    if !self.at(&[")"]) {
                        args.push(self.parse_expr()?);
    ...
    fn parse_primary(&mut self) -> Result<Expr, ParseError> {
        ...
        if matches!(t.kind, TokKind::Ident | TokKind::Kw)
            && !reserved.contains(&t.text.as_str())
            && (t.kind == TokKind::Ident || t.text == "console")
        {
            self.advance();
            return Ok(Expr::Name {
                name: t.text,
