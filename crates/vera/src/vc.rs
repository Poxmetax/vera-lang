...
// [GAP-D2] serde derives are additive (no existing surface serializes a
// Discharge): durable proof certificates round-trip verdicts through disk.
...
fn discharge_goal(
    ...
    let query = build_query(&decls, &assumptions, result_term, binder_aliases, &goal_smt);
...
/// [GAP-D2-SOLVER-SKIP] Single source of truth for fn-level obligation
/// target strings: the cache-skip engine (`store.rs`) reconstructs
/// obligations from cached verdicts and must stay byte-identical with the
/// fresh path (`format_report` prints these verbatim).
pub(crate) fn ensures_target(fn_name: &str, index: usize) -> String {
    format!("{fn_name} ensures[{index}]")
}

/// [GAP-D2-SOLVER-SKIP] Target string of a return-refine obligation on the
/// discharge path (the body-encode-fail path keeps its shorter historical
/// form -- never persisted, so never reconstructed from cache).
pub(crate) fn return_refine_target(fn_name: &str, binder: &str) -> String {
    format!("{fn_name} return refine {{{binder}: Int | …}}")
}
...
fn prove_fn(fn_decl: &FnDecl, out: &mut Vec<Obligation>) {
    let result_term = match encode_block(&fn_decl.body) {
        ...
        Err(reason) => {
            for (i, _) in fn_decl.ensures.iter().enumerate() {
                out.push(Obligation {
                    target: ensures_target(&fn_decl.name, i),
                    kind: "ensures".into(),
    ...
    } = &fn_decl.ret
    {
        ...
        let alias = match sanitize_sym(name) {
            ...
            Err(reason) => {
                out.push(Obligation {
                    target: return_refine_target(&fn_decl.name, name),
                    kind: "return_refine".into(),
                    status: Discharge::RuntimeChecked { reason },
                    span: Some(fn_decl.span),
                    fn_name: Some(fn_decl.name.clone()),
...
/// [GAP5-INV2] The INV-2 cache-key scheme: every persisted result (proof
/// certificate, FixPatch, memo) must be keyed by content hash PLUS toolchain
/// identity so an upgrade can never serve a stale verdict (SPEC INV-2 / §6.4).
/// D's in-process ProvedSet intentionally carries no key: it dies with the
/// process.
#[derive(Debug, Clone, PartialEq, Eq, Hash, Serialize, Deserialize)]
...
/// [GAP-D2-SOLVER-SKIP] Fn-level obligations (`ensures` / `return_refine`)
/// for ONE declaration, exactly as `prove_program` produces them for that
/// fn (Z3 runs). Exposed so the cache-skip engine (`store.rs`) can prove a
/// single fn fresh when its INV-2 lookup misses.
pub fn prove_fn_obligations(fn_decl: &FnDecl) -> Vec<Obligation> {
    let mut out = Vec::new();
    prove_fn(fn_decl, &mut out);
    out
}

/// [GAP-D2-SOLVER-SKIP] Call-site obligations for the whole program,
/// exactly as `prove_program` produces them (Z3 runs). Never cached or
/// skipped: a call-site obligation depends on BOTH caller and callee, so a
/// single definition-hash key would be unsound (the GAP-D2-CLI exclusion).
pub fn prove_call_obligations(program: &Program) -> Vec<Obligation> {
    let mut out = Vec::new();
    prove_calls(program, &mut out);
    out
}
...
mod tests {
    ...
    fn r2_deep_nesting_is_parse_error_not_overflow() {
        ...
        assert!(
            parse(&deep).is_err(),
            "over-deep nesting must be a ParseError"
        );
...
"#;
        ...
        assert!(
            obs.iter().all(|o| !matches!(o.status, Discharge::Proved)),
            "{obs:?}"
        );
    }
