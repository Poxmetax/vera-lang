...
impl<'a> Interpreter<'a> {
    ...
    fn call_fn(&mut self, fn_decl: &FnDecl, args: Vec<Value>) -> Result<Value, Trap> {
        ...
        let mut env = Env::new();
        for (p, a) in fn_decl.params.iter().zip(args) {
        ...
        match expr {
            ...
            Expr::FieldAccess { obj, field, .. } => {
                ...
                match o {
                    ...
                    Value::Console => Err(Trap("bare Console field — call .print(...)".into())),
                    Value::List(_) | Value::Int(_) => {
                        Err(Trap(format!("bare method {field} — call .{field}(...)")))
                    }
    ...
    match (a, b) {
        ...
        (
            ...
            n1 == n2
                ...
                && f1
                    .iter()
                    .all(|(k, v)| f2.get(k).is_some_and(|w| values_eq(v, w)))
        }
        (
            Value::Enum {
                type_name: t1,
...
"#;
        ...
        assert!(
            proved.return_refine_proved("clamp"),
            "clamp return refine must be proved"
        );
...
"#;
        let (prog, proved) = prove_and_build(src);
        assert!(
            !proved.ensures_proved("half", 0),
            "unencodable ensures must not be proved"
        );
        let mut interp = Interpreter::with_proved(&prog, proved);
        let err = interp
            .run_main()
            .expect_err("violated unproved ensures must trap");
...
"#;
        let (prog, proved) = prove_and_build(src);
        assert!(
            !proved.ensures_proved("bump", 0),
            "refuted ensures must not be proved"
        );
        let mut interp = Interpreter::with_proved(&prog, proved);
        let err = interp
            .run_main()
            .expect_err("refuted ensures must still trap at runtime");
        assert!(err.0.contains("ensures violated"), "{err}");
        assert_eq!(interp.elided_checks, 0);
    }
